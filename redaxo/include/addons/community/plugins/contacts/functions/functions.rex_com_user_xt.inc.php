	...
	if ($u->getRows()==1)
	{
		...
		if (is_object($REX['COM_USER']))
		{
			...
			if ($k->getRows()==1)
			{
				...
				{
					$msg = "Kontakt wurde gel&ouml;scht.";
			...
			}else
			{
				...
				if ($_REQUEST["add_contact"]==1)
				{
					...
					if ($user_id != $REX['COM_USER']->getValue('rex_com_user.id'))
					{
						rex_com_user_xt::exeAction($user_id,"sendemail_contactrequest", 
							array(
							"user_id" => $REX['COM_USER']->getValue('rex_com_user.id'),
							"firstname" => $REX['COM_USER']->getValue('rex_com_user.firstname'),
							"name" => $REX['COM_USER']->getValue('rex_com_user.name'),
		...
		if ($MY)
		{
			...
			$buttons[] = '<a href="'.rex_getUrl(REX_COM_PAGE_MYPROFIL_ID,0,array("tab"=>2)).'"><span>Pers&ouml;nliches bearbeiten</span></a>';
			$buttons[] = '<a href="'.rex_getUrl(REX_COM_PAGE_MYPROFIL_ID,0,array("tab"=>3)).'"><span>Einstellungen</span></a>';
			$buttons[] = '<a href="'.rex_getUrl(REX_COM_PAGE_MYPROFIL_ID,0,array("tab"=>4)).'"><span>Passwort &auml;ndern</span></a>';
		}
		...
		$tab_arr_in = array(
			...
			'Pers&ouml;nliches' => 'REX_LINK_ID[3]', 
			'G&auml;stebuch' => 'REX_LINK_ID[4]',
			);
		...
		switch($tab_g)
		{
			...
			case("3"):
				...
				echo rex_com_guestbook::getGuestbook($user_id,$aid,array("tab"=>$tab_g));
...
function rex_com_showUser($sql, $style = "mini", $table = "", $linked = TRUE)
{
	...
	{
		$return = 'index.php?article_id='.$REX["ADDON"]["community"]["settings"]["REX_COM_PAGE_PROFIL_ID"].'&user_id='.$sql->getValue($table.".id");
		$linked = FALSE;
	}else
	{
	...
	if ($linked)
	{
		if (isset($REX['COM_USER']) AND is_object($REX['COM_USER'])) $return = '<a href="'.rex_getUrl($REX["ADDON"]["community"]["settings"]["REX_COM_PAGE_PROFIL_ID"], '',array("user_id"=>$sql->getValue($table.".id"))).'" class="'.$admin_class.'">'.$return.'</a>';
	}
