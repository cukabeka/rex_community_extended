  ...
  class rex_xform_manager
  {
    ...
    public function getDataPage()
    {
      ...
      if($show_editpage) {
        ...
        if(!$popup && $func == "import" && $this->hasDataPageFunction("import")) {
          ...
          echo '<br class="rex-clearer" />
          <div class="rex-navi-paginate rex-toolbar">
            <div class="rex-toolbar-content" style="/*padding:5px*/">
              <a href="index.php?'.$link_vars.$em_url.$em_rex_list.'"><b>'.$I18N->msg('back_to_overview').'</b></a>
            </div>
          </div>'.PHP_EOL;
        ...
        {
          $back = '<br class="rex-clearer" />
          <div class="rex-navi-paginate rex-toolbar">
            <div class="rex-toolbar-content" style="/*padding:5px*/">
              <a href="index.php?'.$link_vars.$em_url.$em_rex_list.'"><b>&laquo; '.$I18N->msg('back_to_overview').'</b></a>
            </div>
          </div>'.PHP_EOL;
      ...
      // *********************************************
          ...
          // RIGHT FLOAT BLOCK
          ////////////////////////////////////////////////////////////////////////////
          echo '<span style="float:right;">Tabelle: ';

          // TABLEMANAGER ADMIN LINK
          if($REX['USER']->isAdmin())
          {
            echo '<a href="index.php?page=xform&subpage=manager&tripage=table_field&table_name='.$table["table_name"].'">'.$I18N->msg("manage_table").'</a>';
          }

          // INFO LINK
          echo ' | <a href="#" id="infotoggler">'.$I18N->msg("pool_file_details").'</a>';
          ...
          echo '</span><br class="rex-clearer" /></div>';
          ...
          // INFOBLOCK
          echo '
          <div id="infoblock" style="display:none;/*padding:10px;*/"  class="">
          <h3 class="rex-hl2" style="font-size:12px;font-weight:bold;">'.$I18N->msg("pool_file_details").'</h3>
          <ul>';
          ...
          echo '</ul></div><br class="rex-clearer" />';

          echo '</div><!-- .rex-addon-output -->';
    ...
    function getFieldPage()
    {
      ...
      if( ($func == "add" || $func == "edit" )  && isset($types[$type_id][$type_name]) )
      {
        ...
        //////////////////////////////////////////////////////////////////////////////
        $prio_qry = $func == 'add'
                  ? 'SELECT `prio`-1 as id ,CONCAT(`prio` , " – " , `f2` , " – [" , `f1` , "]") as name from `rex_xform_field` WHERE `table_name`="'.$table['table_name'].'" ORDER BY `prio` ASC'
                  : 'SELECT `prio`   as id ,CONCAT(`prio` , " – " , `f2` , " – [" , `f1` , "]") as name from `rex_xform_field` WHERE `table_name`="'.$table['table_name'].'" ORDER BY `prio` ASC';
        $xform->setValueField('select_sql', array('prio', 'Prioritaet', $prio_qry));
      ...
      if($func == "list"){
        ...
        if($show_list) {
          ...
          // RUN PRIO MANAGER -> SANITIZE PRIOS
          ////////////////////////////////////////////////////////////////////////////
          $SPM = new sql_prio_manager('rex_xform_field','prio','`table_name`="'.$table["table_name"].'"',true);
          unset($SPM);

          // CUSTOM LIST VIEW FORMATTER FUNCTIONS
          ////////////////////////////////////////////////////////////////////////////
          function rex_xform_list_format($p, $value = "")
          {
            ...
            switch($p["list"]->getValue('type_id'))
            {
              ...
              default: // value
                switch ($p["list"]->getValue('type_name'))
                {
                  case 'fieldset':
                    $style = 'background-color:#dfe9e9;';
                    break;
            ...
            switch($p['field']){
              case'search':
                $p['value'] = $p['value']==1 ? '<span style="color:green">true</span>' : '<span style="color:#dedede;">false</span>' ;
                break;
              case'list_hidden':
                $p['value'] = $p['value']==1 ? '<span style="color:#dedede;">hidden</span>' : '<span style="color:green;">shown</span>' ;
                break;
              default:
                // go on..
                break;
          ...
          $sel = new rex_select();
          $sel->setSize(1);
          $sel->setName('table_jump');
          $sel->setId('table_jump');
          $sel->addSqlOptions('SELECT CONCAT(table_name, " - ", name),table_name FROM rex_xform_table ORDER BY `prio` ASC');
          $sel->setSelected(rex_request('table_name','string'));
          $sel->setAttribute('style','margin:-3px 0 0 0;width:auto;');
          ...
          <div class="rex-navi-paginate rex-toolbar">
            <div class="rex-toolbar-content">
              <b>Feld:</b>
                      <a href=index.php?'.$link_vars.'&table_name='.$table["table_name"].'&func=choosenadd><b>'.$I18N->msg("add").'</b></a>
              <span style="float:right">
              <b>'.$table["table_name"].':</b>
                      <a href="index.php?page=xform&subpage=manager&tripage=data_edit&table_name='.$table["table_name"].'"><b>aufrufen</b></a>
                    | <b>wechseln zu:</b> '.$table_select.'
              </span>
            </div>
          </div>
          <script>
            // TABLE JUMP
            jQuery("#table_jump").change(function(){
              window.location = "index.php?page=xform&subpage=manager&tripage=table_field&table_name="+jQuery(this).val();
              return true;
            });
          </script>
          ';
          ...
          $list->setColumnLabel('list_hidden','list');
          $list->setColumnLabel('type_id','type');
          $list->setColumnLabel('type_name','class');
          $list->setColumnLabel('f1','field');
          $list->setColumnLabel('f2','label');

          $list->setColumnSortable('prio'   );
          $list->setColumnSortable('type_id'   );
          $list->setColumnSortable('type_name'   );
          $list->setColumnSortable('list_hidden'   );
          $list->setColumnSortable('search'   );
          $list->setColumnSortable('f1'   );
          $list->setColumnSortable('f2'   );
          ...
          $list->setColumnLayout('f1', array('<th>###VALUE###</th>','###VALUE###')); // ###VALUE###
          $list->setColumnFormat('f1', 'custom', 'rex_xform_list_format' );
          $list->setColumnLayout('f2', array('<th>###VALUE###</th>','###VALUE###')); // ###VALUE###
          $list->setColumnFormat('f2', 'custom', 'rex_xform_list_format' );
          $list->setColumnLayout('list_hidden', array('<th>###VALUE###</th>','###VALUE###')); // ###VALUE###
          $list->setColumnFormat('list_hidden', 'custom', 'rex_xform_list_format' );
          $list->setColumnLayout('search', array('<th>###VALUE###</th>','###VALUE###')); // ###VALUE###
          $list->setColumnFormat('search', 'custom', 'rex_xform_list_format' );


          //$list->setColumnFormat('list_hidden', 'custom', 'rex_xform_list_format' );
          //$list->setColumnFormat('search', 'custom', 'rex_xform_list_format' );

          for($i=3;$i<10;$i++){ $list->removeColumn('f'.$i); }
          ...
          $list->addLinkAttribute($I18N->msg("delete"), 'class', 'delete-data');

          echo $list->get();

          // ADMIN ONLY: REGENERATE/UPDATE TABLES BLOCK
          if($REX['USER']->isAdmin()){
            echo '
            <div class="rex-navi-paginate rex-toolbar" style="margin-top:10px;">
              <div class="rex-toolbar-content">
                <span>&nbsp;</span>
                <span style="float:right">
                <b>'.$table["table_name"].':</b>
                    <a class="updatetable" href="index.php?'.$link_vars.'&table_name='.$table["table_name"].'&func=updatetable"><b>'.$I18N->msg("updatetable").'</b></a>
                  | <a class="updatetable_with_delete" href="index.php?'.$link_vars.'&table_name='.$table["table_name"].'&func=updatetablewithdelete"><b>'.$I18N->msg("updatetable_with_delete").'</b></a>
                </span>
              </div>
            </div>
            ';
